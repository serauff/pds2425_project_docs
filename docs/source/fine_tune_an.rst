...
shown in the subsection :ref:`_Dataset Annotation`. Every context has a single answer to a single question. The necessary structure for fine-tuning QA models is thus provided as
...
To finally generate a large datset which we can later use for fine-tuning, we use similar methods as in the :ref:`_Prompting Gemini via API:` section. However, instead of first generating full-text questions with `gemini-1.5-flash` we utilise the model to create new json questionnaires in the format of the ones provided by snapADDY. For this, the :py:func:`data_gen.init_context` is utilised. 
...
Then we use the same logic as described in :doc:`data_generation` to create new Question-Answer pairs with our synthetic questionnaires. This results in 2266 Question-Answer pairs that are ready to be annotated by our fine-tuned models in the :ref:`_annotator:` section.
